...
/*   Updated: 2022/06/22 09:27:58 by aozsayar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	checkcolission(t_character *e, t_character *c, int size)
{
	...
	cposx = c->positonx;
	cposy = c->positony;
	if (((eposx + size > cposx && eposx <= cposx)
			|| (eposx < cposx + size && eposx + size > cposx + size))
		&& ((eposy <= cposy && eposy + size > cposy)
			|| (eposy < cposy + size && eposy + size > cposy + size)))
	{
		if (e->iscatch == CATCH)
			c->isdied = DIED;
		else
